...
(function () {
  ...
  xtte.project.save = function(prjId)
  {
    ...
    var params = { teprj_id: _teprjid, prj_id: prjId };
    if (_cust.isValid())
  ...
  {
    var q = toolbox.executeDbQuery("project", "selteprj", { prj_id: _prjid });
  ...
  xtte.project.newTask = function()
  {
    if (_prjid == -1)
    {
      ...
      {
        var q = toolbox.executeDbQuery("projectGantt", "getprj",
                                       { prjNumber: _number.text });
        if (q.first())
  ...
  xtte.project.gantt = function()
  {
    ...
    var qry = toolbox.executeDbQuery("project", "formatdates",
                                     { startDate: _started.date,
                                       dueDate:   _due.date });
    if (qry.first())
    {
      var params = { prjNumber: _number.text,
                     prjName:   _name.text,
                     startDate: qry.value("start_date"),
                     dueDate:   qry.value("due_date")
      };
